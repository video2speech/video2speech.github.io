...
def parse_frequency_file(filename):
    ...
    for line in lines[2:]:  # Skip header lines
        if line.strip() and not line.startswith('\t'):
            # Split by tabs and filter empty strings
            parts = [p.strip() for p in line.split('\t') if p.strip()]
            if len(parts) >= 4:  # Need at least word, pos, freq, and more
                word = parts[0].lower()  # First column is word
                try:
                    freq_spoken = int(parts[2])  # Third column is FrSp
                    if word and word not in ["word", "frsp"]:  # Skip headers
