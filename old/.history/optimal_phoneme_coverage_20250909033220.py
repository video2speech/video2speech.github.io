...
def parse_frequency_file(filename):
    ...
    for line_num, line in enumerate(lines[2:], 3):  # Skip first 2 lines (header and empty)
        if line.strip():
            # Split by tabs
            parts = line.split('\t')
            if len(parts) >= 6:  # Format: \tword\tpos\tfreq\t...\tfreq2
                word = parts[1].strip().lower()  # Second column is word (after initial tab)
                try:
                    freq_spoken = int(parts[3])  # Fourth column is FrSp (frequency spoken)
