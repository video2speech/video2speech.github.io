...
def analyze_phoneme_word_counts(word_to_phonemes: Dict[str, Set[str]]) -> Tuple[Dict[str, List[str]], Dict[str, List[str]], Dict[str, List[str]], Dict[str, List[str]]]:
    """
    Find phonemes with limited word representations.
    Returns (single_word_phonemes, dual_word_phonemes, triple_word_phonemes, quad_word_phonemes).
    """
    ...
    for phoneme, words in phoneme_to_words.items():
        if len(words) == 1:
            ...
            quad_phoneme_words[phoneme] = words
    
    return single_phoneme_words, dual_phoneme_words, triple_phoneme_words, quad_phoneme_words
...
def main():
    ...
    print("Finding minimal coverage with at least 4 words per phoneme...")
    k, top_k_words, word_to_phonemes = find_minimal_coverage_with_min_words(word_frequencies, min_words_per_phoneme=4)
    ...
    print(f"\nMinimum k = {k}")
    print(f"Top {k} words that ensure each phoneme is represented by at least 4 words:")
    print()
    ...
    single_phoneme_words, dual_phoneme_words, triple_phoneme_words, quad_phoneme_words = analyze_phoneme_word_counts(word_to_phonemes)
    
    print("Final word list (duplicates removed):")
    for i, word in enumerate(unique_words, 1):
        phonemes = word_to_phonemes[word]
        
        # Check if this word contains any limited representation phonemes
        markers = []
        for phoneme in phonemes:
            ...
            elif phoneme in triple_phoneme_words:
                markers.append(f"1of3-{phoneme}")
            elif phoneme in quad_phoneme_words:
                markers.append(f"1of4-{phoneme}")
    ...
    if triple_phoneme_words:
        print("\nPhonemes with EXACTLY 3 word representations:")
        for phoneme, words in triple_phoneme_words.items():
            print(f"  '{phoneme}' -> in words: {', '.join(words)}")
    
    if quad_phoneme_words:
        print("\nPhonemes with EXACTLY 4 word representations:")
        for phoneme, words in quad_phoneme_words.items():
            print(f"  '{phoneme}' -> in words: {', '.join(words)}")
    
    if not single_phoneme_words and not dual_phoneme_words and not triple_phoneme_words and not quad_phoneme_words:
        print("All phonemes are represented by 5+ words.")
