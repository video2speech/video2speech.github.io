...
class PhonemeTracker:
    ...
    def remove_word(self, word_or_number):
        """从选中列表移除词汇（支持词汇名或编号）"""
        # 尝试按编号删除
        if word_or_number.isdigit():
            index = int(word_or_number) - 1  # 转换为0-based索引
            if 0 <= index < len(self.selected_words):
                removed_word = self.selected_words.pop(index)
                self.save_selected_words()
                print(f"🗑️  移除成功: #{word_or_number} {removed_word}")
                return True
            else:
                print(f"❌ 编号无效: {word_or_number} (范围: 1-{len(self.selected_words)})")
                return False
        
        # 按词汇名删除
        if word_or_number in self.selected_words:
            self.selected_words.remove(word_or_number)
            self.save_selected_words()
            print(f"🗑️  移除成功: {word_or_number}")
            return True
        else:
            print(f"❌ 词汇不存在: {word_or_number}")
            return False
    ...
    def run_interactive(self):
        ...
        print("💡 使用说明:")
        print("   📝 直接输入词汇 → 添加到列表")
        print("   🗑️  remove <词汇/编号> → 删除指定词汇")
        print("   📊 show → 显示当前状态")
        print("   📋 list → 列出所有选中词汇")
        print("   🧹 clear → 清空所有词汇")
        print("   👋 quit → 退出程序")
        print("   💡 删除例子: remove the 或 remove 1")
