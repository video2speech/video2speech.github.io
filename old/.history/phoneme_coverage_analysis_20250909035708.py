...
def main():
    ...
    print(f"\nMinimum k = {k}")
    print(f"Top {k} words that ensure each phoneme is represented by at least 3 words:")
    print()
    ...
    single_phoneme_words, dual_phoneme_words, triple_phoneme_words = analyze_phoneme_word_counts(word_to_phonemes)
    
    print("Final word list (duplicates removed):")
    for i, word in enumerate(unique_words, 1):
        phonemes = word_to_phonemes[word]
        
        # Check if this word contains any single, dual, or triple representation phonemes
        markers = []
        for phoneme in phonemes:
    ...
    if not single_phoneme_words and not dual_phoneme_words and not triple_phoneme_words:
        print("All phonemes are represented by 4+ words.")
