...
def get_english_phonemes() -> Set[str]:
    ...
    # Standard English phonemes in ARPABET notation (44 phonemes total)
    consonants = {
    ...
    # Just use the standard 39 phonemes that are actually found in CMU dict
    # The "44 phonemes" often includes allophones not distinguished in ARPABET
...
def get_actual_phonemes_in_cmu(cmu_dict: Dict[str, List[List[str]]]) -> Set[str]:
    """Get all phonemes that actually appear in the CMU dictionary"""
    all_phonemes = set()
...
def main():
    ...
    # Get actual phonemes in CMU dict
    actual_phonemes = get_actual_phonemes_in_cmu(cmu_dict)
    print(f"Phonemes found in CMU dict ({len(actual_phonemes)}): {sorted(actual_phonemes)}\n")
    
    # Use the standard 44 phonemes that intersect with what's in CMU dict
    standard_phonemes = get_english_phonemes()
    target_phonemes = standard_phonemes & actual_phonemes
    print(f"Target phonemes to cover ({len(target_phonemes)}): {sorted(target_phonemes)}\n")
