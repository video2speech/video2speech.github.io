...
def process_movie_lines(movie_file, vocab_file, output_file, max_lines=None):
    ...
    if valid_sentences:
        ...
        print(f"  平均: {sum(lengths)/len(lengths):.2f} 词")
        
        # 长度分布
        short = sum(1 for l in lengths if l <= 5)
        medium = sum(1 for l in lengths if 6 <= l <= 10)
        long = sum(1 for l in lengths if l > 10)
        
        print(f"  短句 (≤5词): {short} ({short/len(lengths)*100:.1f}%)")
        print(f"  中句 (6-10词): {medium} ({medium/len(lengths)*100:.1f}%)")
        print(f"  长句 (>10词): {long} ({long/len(lengths)*100:.1f}%)")
    
    # 保存有效句子
    print(f"\n正在保存有效句子到: {output_file}")
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(f"# 电影台词 - 仅包含新高频词汇表的句子 (无长度限制)\n")
    ...
    with open(stats_file, 'w', encoding='utf-8') as f:
        ...
        if valid_sentences:
            ...
            f.write("长度分布:\n")
            f.write(f"  短句 (≤5词): {short} ({short/len(lengths)*100:.1f}%)\n")
            f.write(f"  中句 (6-10词): {medium} ({medium/len(lengths)*100:.1f}%)\n")
            f.write(f"  长句 (>10词): {long} ({long/len(lengths)*100:.1f}%)\n\n")
            
            # 显示几个示例
            f.write("有效句子示例:\n")
            for i, item in enumerate(valid_sentences[:15], 1):
                f.write(f"{i:2d}. ({item['token_count']:2d}词) {item['sentence'][:80]}{'...' if len(item['sentence']) > 80 else ''}\n")
