...
def analyze_word_frequency_in_sentences(filename):
    """分析句子文件中各个词的出现频率"""
    
    print("=" * 80)
    print(f"分析 {filename} 中的词频")
    print("=" * 80)
    ...
    try:
        ...
        # 保存详细结果到文件
        output_file = filename.replace('.txt', '_word_frequency.txt')
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"词频分析结果 - {filename}\n")
            f.write("=" * 60 + "\n\n")
        ...
        # 保存CSV格式
        csv_file = filename.replace('.txt', '_word_frequency.csv')
        ...
        return sorted_words, word_counter
...
def main():
    """主函数"""
    filename = 'materials/200_sentences_list.txt'
    sorted_words, word_counter = analyze_word_frequency_in_sentences(filename)
