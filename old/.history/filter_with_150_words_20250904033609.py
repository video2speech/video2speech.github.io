...
def process_movie_lines(movie_file, vocab_file, output_file, max_lines=None):
    ...
    with open(stats_file, 'w', encoding='utf-8') as f:
        f.write("电影台词词汇过滤统计报告 - 150_words_list.txt\n")
        f.write("=" * 60 + "\n\n")
        f.write(f"源文件: {movie_file}\n")
        f.write(f"词汇表: {vocab_file}\n")
        f.write(f"词汇表大小: {len(vocab)} 个词\n")
        f.write(f"长度限制: 必须≥4词（不包括标点符号）\n\n")
        f.write(f"处理行数: {len(df)}\n")
        f.write(f"有效句子: {len(unique_sentences)}\n")
        f.write(f"无效句子: {len(invalid_sentences)}\n")
        f.write(f"有效率: {len(unique_sentences)/(len(unique_sentences)+len(invalid_sentences))*100:.2f}%\n\n")
...
def main():
    ...
    output_file = "filtered_sentences_150_words.txt"
